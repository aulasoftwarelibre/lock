...
###############################################
# "php" stage
FROM php:${PHP_VERSION}-fpm-alpine AS lock_php
...
ENV APP_ENV=prod
ENV ROUTER_REQUEST_CONTEXT_HOST=lock.aulasoftwarelibre.uco.es
...
###############################################
# "apache" stage
FROM httpd:${APACHE_VERSION} AS lock_httpd
...
COPY --from=lock_php /srv/app/public public/
